...
class ForeignKey extends ForeignKeyMiddle
{
    ...
    {
        $readI = new InfoTable();
        $readI->ExeRead("KEY_COLUMN_USAGE", "WHERE TABLE_SCHEMA = :nb && REFERENCED_TABLE_SCHEMA = :nb && TABLE_NAME =:tn && COLUMN_NAME = :tb", "nb={$this->db}&tn={$this->table}&tb={$this->column}", !0);
        if ($readI->getResult()):
    ...
    private function getPk($table)
    {
        ...
        $db = DATABASE;
        $readI = new InfoTable();
        $readI->ExeRead("COLUMNS", "WHERE TABLE_SCHEMA = :nb && TABLE_NAME = :nt", "nb={$db}&nt={$table}", !0);
        if ($readI->getResult()):
            foreach ($readI->getResult() as $g):
                if ($g['COLUMN_KEY'] === "PRI"):
                    return true;
    ...
    {
        $readI = new InfoTable();
        $readI->ExeRead("KEY_COLUMN_USAGE", "WHERE TABLE_SCHEMA = :nb && REFERENCED_TABLE_SCHEMA = :nb && REFERENCED_TABLE_NAME != '" . PRE . "user' && TABLE_NAME = :tb", "nb={$this->db}&tb={$this->table}", !0);
        if ($readI->getResult()):
            foreach ($readI->getResult() as $g):
                $readI->ExeRead("REFERENTIAL_CONSTRAINTS", "WHERE CONSTRAINT_SCHEMA = :nb && CONSTRAINT_NAME = :cn", "nb={$this->db}&cn={$g['CONSTRAINT_NAME']}", !0);
    ...
    {
        $readI = new InfoTable();
        $readI->ExeRead("KEY_COLUMN_USAGE", "WHERE TABLE_SCHEMA = :nb && REFERENCED_TABLE_SCHEMA = :nb && TABLE_NAME != '" . PRE . "user' && REFERENCED_TABLE_NAME = :tb", "nb={$this->db}&tb={$this->table}", !0);
        if ($readI->getResult()):
            foreach ($readI->getResult() as $g):
    ...
    {
        $readI = new InfoTable();
        $readI->ExeRead("KEY_COLUMN_USAGE", "WHERE TABLE_SCHEMA = :nb && REFERENCED_TABLE_SCHEMA = :nb && REFERENCED_TABLE_NAME = :tb && TABLE_NAME != :tb", "nb={$this->db}&tb={$this->table}", !0);
        if ($readI->getResult()):
            foreach ($readI->getResult() as $g):
    ...
    private function getRelationTableManyToMany($table, $column)
    {
        if (!$this->getPk($table) && $this->acceptThisFluxo($table, $column)):
            $readI = new InfoTable();
            $readI->ExeRead("KEY_COLUMN_USAGE", "WHERE TABLE_SCHEMA = :nb && REFERENCED_TABLE_SCHEMA = :nb && TABLE_NAME = :tn", "nb={$this->db}&tn={$table}", !0);
