...
class ForeignKey extends ForeignKeyMiddle
{
    ...
    {
        $readI = new InfoTable();
        $readI->exeRead("KEY_COLUMN_USAGE", "WHERE TABLE_SCHEMA = :nb && REFERENCED_TABLE_SCHEMA = :nb && REFERENCED_TABLE_NAME != 'user' && TABLE_NAME = :tb", ["nb" => $this->db, "tb" => $this->table]);
        if ($readI->getResult()):
            foreach ($readI->getResult() as $g):
    ...
    {
        $readI = new InfoTable();
        $readI->exeRead("KEY_COLUMN_USAGE", "WHERE TABLE_SCHEMA = :nb && REFERENCED_TABLE_SCHEMA = :nb && TABLE_NAME != 'user' && REFERENCED_TABLE_NAME = :tb", ["nb" => $this->db, "tb" => $this->table]);
        if ($readI->getResult()):
            foreach ($readI->getResult() as $g):
