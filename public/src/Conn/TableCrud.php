...
class TableCrud
{
    ...
    public function load($attr, $value = null)
    {
        ...
        $this->exist = false;
        $read = new Read();
        $read->ExeRead($this->table, "WHERE {$attr} = '" . str_replace("'", "''", $value) . "'", null, !0, !0);
        if ($read->getResult()):
            $this->exist = true;
    ...
    public function loadSql($sql, $arr = null)
    {
        $this->exist = false;
        $read = new Read();
        $read->ExeRead($this->table, "WHERE {$sql}", null, !0, !0);
        if ($read->getResult()):
            $this->exist = true;
            foreach ($read->getResult()[0] as $key => $value):
                $this->colunas[] = $key;
    ...
    private function readNewTableColumns()
    {
        if (!$this->colunas):
            $db = DATABASE;
            $read = new InfoTable();
            $read->exeRead("COLUMNS", "WHERE TABLE_SCHEMA = :nb && TABLE_NAME = :nt", "nb={$db}&nt={$this->table}", !0, !0);
            if ($read->getResult()):
